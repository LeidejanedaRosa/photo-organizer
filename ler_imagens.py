...
def executar_opcao(opcao: int, diretorio: str):
    ...
    if opcao == 1:
        ...
        # Filtra imagens com datas válidas para organização por anos
        imagens_com_data = []
        for img in imagens_todas:
            data_foto = img.data_exif or img.data_mod
            ano_bebe = calcular_ano_bebe(data_foto)
            if ano_bebe > 0:
                imagens_com_data.append(img)
        
        if imagens_com_data:
            # Agrupa por anos para mostrar estatísticas
            imagens_por_ano = defaultdict(list)
            for img in imagens_com_data:
                data_foto = img.data_exif or img.data_mod
                ano_bebe = calcular_ano_bebe(data_foto)
                imagens_por_ano[ano_bebe].append(img.arquivo)
            ...
            # Mostra simulação
            organizar_por_anos_automatico(imagens_com_data, diretorio, simular=True)
